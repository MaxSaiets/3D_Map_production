...
const COPY: Record<AppLocale, KarpatyCopy> = {
  uk: {
    ...
    h1: "Карта Карпат у 3D — рельєфна модель гір",
    ...
    faq: [
      ...
      { q: "Скільки коштує і як швидко?", a: `Настільна рельєфна мапа — від ${M[80] + R} ₴ (розмір M, 8 см). Топо-брелок — від ${KEYCHAIN_PRICE_UAH} ₴. Виготовлення 2–4 робочі дні, доставка Новою Поштою по Україні.` },
    ],
  ...
  en: {
    ...
    how: [
      ...
      "Order the print — made in 2–4 business days, shipped across Ukraine.",
    ],
    h2faq: "FAQ",
    faq: [
      ...
      { q: "Price and lead time?", a: `A desk relief map from ≈€${eur(M[80] + R)} (size M, 8 cm). Topo keychain from ≈€${eur(KEYCHAIN_PRICE_UAH)}. Production 2–4 business days, shipping across Ukraine.` },
    ],
  ...
  de: {
    ...
    faq: [
      ...
      { q: "Preis und Lieferzeit?", a: `Tisch-Reliefkarte ab ≈${eur(M[80] + R)} € (Größe M, 8 cm). Topo-Anhänger ab ≈${eur(KEYCHAIN_PRICE_UAH)} €. Fertigung 2–4 Werktage, Versand innerhalb der Ukraine.` },
    ],
  ...
  pl: {
    ...
    faq: [
      ...
      { q: "Cena i termin?", a: `Mapa na biurko z rzeźbą od ≈${eur(M[80] + R)} € (rozmiar M, 8 cm). Brelok topo od ≈${eur(KEYCHAIN_PRICE_UAH)} €. Wykonanie 2–4 dni robocze, wysyłka po Ukrainie.` },
    ],
  ...
  fr: {
    ...
    faq: [
      ...
      { q: "Prix et délais ?", a: `Carte relief de bureau dès ≈${eur(M[80] + R)} € (taille M, 8 cm). Porte-clés topo dès ≈${eur(KEYCHAIN_PRICE_UAH)} €. Fabrication 2–4 jours ouvrés, livraison en Ukraine.` },
    ],
  ...
  es: {
    ...
    faq: [
      ...
      { q: "¿Precio y plazos?", a: `Mapa de escritorio en relieve desde ≈${eur(M[80] + R)} € (talla M, 8 cm). Llavero topo desde ≈${eur(KEYCHAIN_PRICE_UAH)} €. Fabricación 2–4 días hábiles, envío dentro de Ucrania.` },
    ],
