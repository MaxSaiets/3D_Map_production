...
import { BASE, seoTitle, localeUrl, priceValidUntil, priceValidFrom, MERCHANT_RETURN_POLICY_LD } from "@/i18n/metadata";
...
export default async function BrelokCityPage({
  ...
  const UI: Record<AppLocale, { breadcrumb: string; cta: string; ctaGift: string; districts: string; price: string; others: string; map: string }> = {
    ...
    en: { breadcrumb: "Keychains", cta: "Create a keychain in the builder", ctaGift: "A gift from this city", districts: "Popular districts for a keychain", price: `Price: from ≈€${eur} per keychain · made in 2–4 business days`, others: "Keychains from other cities", map: `3D city map — ${name}` },
    de: { breadcrumb: "Anhänger", cta: "Anhänger im Konfigurator erstellen", ctaGift: "Ein Geschenk aus dieser Stadt", districts: "Beliebte Viertel für einen Anhänger", price: `Preis: ab ≈${eur} € pro Anhänger · Fertigung 2–4 Werktage`, others: "Anhänger aus anderen Städten", map: `3D-Stadtkarte — ${name}` },
