...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  return pageMetadata({ locale: params.locale, path: "/refund", ns: "refundMeta" });
}

export default function RefundPage({ params }: { params: { locale: string } }) {
  const doc = getLegalSet(params.locale).refund;
  return (
    <div className="mx-auto max-w-[760px] px-5 py-12 lg:px-8">
      ...
      <LegalArticle doc={doc} locale={params.locale} path="/refund" />
    </div>
  );
}
