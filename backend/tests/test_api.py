...
def _skip_if_heavy_generation():
    """POST /api/generate з реальними координатами запускає СПРАВЖНЮ генерацію
    (OSM-фетч + важка геометрія) у фоновій задачі, а FastAPI TestClient блокується
    до завершення background-task → тест висів хвилинами і ВЕШАВ увесь прогін.
    Гейтимо як regression-кейси (opt-in RUN_REGRESSION=1), щоб дефолтний прогін
    лишався швидким; контракт ендпойнта (200 + task_id) перевіряється в проді."""
    if os.getenv("RUN_REGRESSION", "0") != "1":
        pytest.skip("triggers real generation (TestClient blocks) — opt-in via RUN_REGRESSION=1")
