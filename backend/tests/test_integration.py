...
# Усі тести цього файлу POST-ять /api/generate і виконують ПОВНИЙ цикл генерації
# (OSM-фетч + важка геометрія). FastAPI TestClient блокується до завершення
# фонової задачі → кожен тест висить хвилинами і ВЕШАЄ увесь прогін. Гейтимо як
# regression-кейси: opt-in через RUN_REGRESSION=1 (дефолтний прогін лишається
# швидким; повний цикл також покривається tests/test_regression_cases.py).
pytestmark = pytest.mark.skipif(
    os.getenv("RUN_REGRESSION", "0") != "1",
    reason="full generation cycle (TestClient blocks on background task) — opt-in via RUN_REGRESSION=1",
)
