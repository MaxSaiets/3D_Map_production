...
async def health():
    ...
    try:
        ...
        pass
    # ⭐09.09.2026: скільки генерацій зараз виконується. Потрібно деплою, щоб
    # НЕ рестартувати бекенд посеред чужої моделі: у логах Caddy за тиждень
    # 377 відповідей 502, і вони збиралися саме у вікна рестартів — 80 поспіль
    # на статус ОДНІЄЇ задачі. Рестарт вбиває задачу в памʼяті, і людина
    # отримує «модель застаріла» замість файлу. Скрипт деплою тепер може
    # дочекатись `queue.running == 0` (лічильники, нічого приватного).
    try:
        from services import gen_queue as _gq
        checks["queue"] = _gq.stats()
    except Exception:  # noqa: BLE001
        pass
    return {
