...
def carve_highlight_groove(
    ...
    *,
    ...
    try:
        ...
        def _drift_ok(_res, _b0):
            if _res is None or len(getattr(_res, "faces", [])) == 0:
                return False
            _b1 = _res.bounds
            _d = (max(abs(_b1[0][i] - _b0[0][i]) for i in (0, 1))
                  + max(abs(_b1[1][i] - _b0[1][i]) for i in (0, 1)))
            return _d < (5.0 / sf)

        # 1) manifold — швидко, ЧИСТІ стінки, потребує ГЕРМЕТИЧНИЙ рельєф.
        if cutter_ok and bool(getattr(terrain_mesh, "is_volume", False)):
        ...
        # 2) Blender boolean — ріже ЧИСТО і на НЕгерметичному рельєфі (саме твої щільні
        #    горбисті зони). Працює тепер, бо виправлено осьовий своп Y/Z у _run_blender_boolean.
        if cutter_ok:
            try:
                ...
                if (bres is not None and len(getattr(bres, "faces", [])) > 0
                        and abs(len(bres.faces) - len(terrain_mesh.faces)) > 4
        ...
        # 3) ОСТАННІЙ резерв: опускання вершин (обривчасто) — лише якщо обидва boolean впали.
