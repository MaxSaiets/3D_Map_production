...
def clip_generated_meshes(
    *,
    ...
    # КРИТИЧНО: рельєф ВЖЕ побудований солідифаєром на полігоні зони (preclipped),
    # тому повторний bbox-кліп тут — ЗАЙВИЙ і РУЙНІВНИЙ: він ще раз ріже герметичний
    # рельєф площиною й лишає різ ВІДКРИТИМ (≈5000 відкритих ребер → негерметична
    # основа, «відсутні стінки», Bambu пропонує repair). Дороги/будинки/вода вже
    # пропускають повторний кліп за цим самим прапором preclipped_to_zone — рельєф
    # робив це БЕЗ перевірки прапора (це й був баг). Тепер ріжемо ЛИШЕ якщо НЕ
    # preclipped (fallback), коли рельєф справді міг вийти за межі.
    if terrain_mesh is not None and not preclipped_to_zone:
