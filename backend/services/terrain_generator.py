...
def _snap_and_extract_boundary_from_clipped(
    ...
    try:
        ...
        # === Corner vertex pre-snap ===
        # Snap the boundary vertex nearest to each polygon corner to the EXACT corner XY.
        # This ensures top_boundary[corner] has byte-identical XYZ to terrain_top.vertices[corner],
        # so merge_vertices() in the solidifier can seal the wall/surface seam at every corner.
        from scipy.spatial import cKDTree as _cKDTree
        _bv_2d = new_vertices[boundary_vertex_indices, :2]
        _corner_tree = _cKDTree(_bv_2d)
        corner_v_indices = {}  # polygon vertex index -> mesh vertex global index
        for _ci in range(n_poly):
            _cxy = poly_verts[_ci]
            _d_c, _local_ci = _corner_tree.query(_cxy, k=1)
            _gv_idx = int(boundary_vertex_indices[_local_ci])
            new_vertices[_gv_idx, 0] = _cxy[0]
            new_vertices[_gv_idx, 1] = _cxy[1]
            corner_v_indices[_ci] = _gv_idx
        # === End corner pre-snap ===
        ...
        def _append_dedup(lst, vert):
            """Append vertex only if different from the last one (avoids duplicate corner vertices)."""
            if lst and np.sum((vert - lst[-1]) ** 2) < 1e-16:
                return
            lst.append(vert)

        for s in range(n_poly):
            # 1. Додаємо ВСІ вершини сегмента (без фільтру t_val).
            # Фільтр > 0.01 / < 0.99 виключав near-corner вершини, залишаючи ~80 відкритих ребер.
            ...
            if np.any(mask):
                ...
                for idx in seg_boundary_indices[order]:
                    _append_dedup(final_boundary_list, snapped_mesh.vertices[idx])

            # 2. Кутова вершина — pre-snapped to exact corner XY, deduplicated
            next_s = (s + 1) % n_poly
            _append_dedup(final_boundary_list, snapped_mesh.vertices[corner_v_indices[next_s]])
...
def create_terrain_mesh(
    ...
    print(f"[TERRAIN] {'='*80}")
    print(f"[TERRAIN] ПОЧАТОК create_terrain_mesh (Inlay Mode support: {road_polygons_for_cutting is not None})")
    ...
    # Flatten roads is mostly unused in Inlay mode, but kept for compatibility
    ...
    # 6. Provider (Create before mesh modification for accurate road generation later)
    provider = TerrainProvider(X, Y, Z, original_Z=Z_original)

    # 4. Створення mesh
    # INLAY LOGIC: Якщо є road_polygons_for_cutting, ми повинні "пробити" Z-grid або видалити вершини
    # Найефективніше - видалити вершини/грані після створення мешу, або використати маску NaN у Z
    ...
    # 5. Підготовка полігону для Solidifier
    # Це ключовий момент для Інтарсії: ми створюємо полігон з дірками
    
    active_polygon = zone_polygon
    if zone_polygon is None or zone_polygon.is_empty:
        # Fallback square
        active_polygon = ShapelyPolygon([
            (min_x, min_y), (max_x, min_y), (max_x, max_y), (min_x, max_y)
        ])

    # ВИПРАВЛЕННЯ: Вирізання доріг з полігону ПЕРЕД solidification НЕ працює,
    # бо solidifier не обробляє дірки. Вирізання тепер відбувається ПІСЛЯ solidification
    # через boolean difference між мешами в main.py (функція cut_roads_from_solid_terrain).
    # Тут залишаємо тільки вирізання з top surface для кращого вигляду.
    if road_polygons_for_cutting is not None and not road_polygons_for_cutting.is_empty:
        print("[TERRAIN] Road polygons provided, but cutting will be done AFTER solidification via mesh boolean difference")
        # Не вирізаємо з полігону, щоб solidifier створив повний solid mesh
        # Вирізання буде виконано після solidification в main.py

    # 6. Обрізання Top Mesh
    # Нам потрібно видалити трикутники, що знаходяться там, де дороги
    
    try:
        if active_polygon is not None and not active_polygon.is_empty:
            
            # ВИПРАВЛЕННЯ 3 (CRITICAL FIX):
            # Ми ПОВНІСТЮ ВІДМІНИЛИ видалення граней (faces) з top surface!
            # Раніше тут видалялися трикутники у місцях, де проходять дороги. Це руйнувало 
            # watertightness (герметичність) мешу. Solidifier генерував стіни та дно, але 
            # "дах" мав дірки. Через це Blender Boolean DIFFERENCE ламався і робив наскрізні діри (roofs).
            # Тепер ми залишаємо рельєф 100% суцільним, а Blender сам об'ємно виріже пази.
            if road_polygons_for_cutting is not None:
                print("[TERRAIN] Skipping top surface face filtering to maintain watertight solid mesh.")
                pass


    except Exception as e:
        print(f"[WARN] Clipping terrain failed: {e}")

    # 7. Solid Creation (Solidifier)
    solid_start = time.time()
    # Передаємо active_polygon, який містить дірки.
    # Solidifier повинен згенерувати стінки для цих дірок.
    
    # ВИПРАВЛЕННЯ: Додано визначення explicit_floor_z
    explicit_floor_z = -base_thickness if elevation_ref_m is not None else None
    
    if not active_polygon.is_valid:
        active_polygon = active_polygon.buffer(0)

    boundary_verts_3d_from_mesh = None
    
    try:
        # Використовуємо active_polygon (може містити дірки для доріг)
        if active_polygon is not None and not active_polygon.is_empty:
            clipped = clip_mesh_to_polygon_planes(mesh, active_polygon, global_center=None)
            ...
            if clipped is not None and len(clipped.vertices) > 0 and len(clipped.faces) > 0:
                ...
                clipped, boundary_verts_3d_from_mesh = _snap_and_extract_boundary_from_clipped(
                    clipped, active_polygon, tolerance=0.5
                )
        ...
        if boundary_verts_3d_final is None and provider is not None:
             try:
                coords = list(active_polygon.exterior.coords)[:-1]
                b2d = np.array(coords)
        ...
        solid_mesh = create_solid_terrain(
            terrain_top=mesh,
            zone_polygon=active_polygon, # Полігон з дірками!
