...
# ============================================================
# КОЛЬОРОВІ ТЕМИ/ПАЛІТРИ (задача #2) — стилізація під «настрій» карти.
# Застосовуються POST-EXPORT через _patch_3mf_colors (перепатч m:colorgroup), тож
# основний шлях експорту НЕ зачеплено (classic = байт-ідентично). Часткові мапи:
# фарбуємо лише тематичні шари, решта лишається дефолтною.
# ============================================================
COLOR_PALETTES: dict = {
    # Skyrim / стара пергаментна мапа — теплий тан, коричневі дороги.
    "sepia": {
        "base": [224, 206, 169, 255], "terrain": [224, 206, 169, 255], "baseback": [224, 206, 169, 255],
        "buildings": [196, 169, 120, 255], "roads": [92, 62, 32, 255],
        "water": [150, 165, 150, 255], "parks": [142, 140, 92, 255], "green": [142, 140, 92, 255],
        "text": [70, 45, 20, 255], "rim": [70, 45, 20, 255], "maplabel": [70, 45, 20, 255], "frame": [70, 45, 20, 255],
    },
    # Нуар — чорно-біла графіка.
    "noir": {
        "base": [236, 236, 236, 255], "terrain": [236, 236, 236, 255], "baseback": [236, 236, 236, 255],
        "buildings": [178, 178, 178, 255], "roads": [15, 15, 15, 255],
        "water": [120, 120, 120, 255], "parks": [150, 150, 150, 255], "green": [150, 150, 150, 255],
    },
    # Blueprint / морська — світло-блакитна основа, глибока синя вода.
    "ocean": {
        "base": [236, 244, 251, 255], "terrain": [236, 244, 251, 255], "baseback": [236, 244, 251, 255],
        "buildings": [150, 182, 212, 255], "roads": [28, 50, 92, 255],
        "water": [20, 92, 172, 255], "parks": [92, 152, 122, 255], "green": [92, 152, 122, 255],
        "text": [22, 42, 82, 255], "rim": [22, 42, 82, 255], "maplabel": [22, 42, 82, 255],
    },
    # GTA-нічна неонова — темна основа, мadженту-дороги, бірюзова вода.
    "neon": {
        "base": [26, 26, 42, 255], "terrain": [26, 26, 42, 255], "baseback": [26, 26, 42, 255],
        "buildings": [62, 62, 92, 255], "roads": [232, 60, 162, 255],
        "water": [40, 200, 222, 255], "parks": [120, 232, 92, 255], "green": [120, 232, 92, 255],
        "text": [240, 240, 255, 255], "rim": [240, 240, 255, 255], "maplabel": [240, 240, 255, 255],
    },
}


def get_palette_color_map(name: str) -> dict:
    """Повертає часткову color-map для палітри (або None для classic/невідомої)."""
    if not name:
        return None
    return COLOR_PALETTES.get(str(name).lower().strip())
...
def export_3mf(
    ...
    # (один колір; форму будинків видно за висотою), вода СИНЯ, зелень ТЕМНІШЕ,
    # дороги ЧОРНІ. Превʼю (Preview3D) = ця ж палітра.
    COLOR_MAP = {
        "base": [242, 242, 242, 255],      # БІЛА основа/рельєф
        "terrain": [242, 242, 242, 255],
        "baseback": [242, 242, 242, 255],
        "roads": [20, 20, 20, 255],        # ЧОРНІ дороги
        "buildings": [242, 242, 242, 255], # БІЛІ будинки (як основа)
        "water": [47, 111, 214, 255],      # СИНЯ вода
        "parks": [92, 150, 92, 255],       # світліша трава
        "green": [92, 150, 92, 255],
        ...
        "frame": [25, 25, 25, 255],     # преміум-рамка (компас/лінійка/координати) = чорна
    }
    ...
    # КОЖЕН шар — окремий <object> у 3MF (separate geometries).
    # Bambu Studio multi-color: на кожен object користувач призначає свій
