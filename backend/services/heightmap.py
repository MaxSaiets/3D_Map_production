    ...
    try:
        ...
        for g in building_geometries:
            for poly in _iter_polygons(g):
                ...
                # Вирівнюємо до МЕДІАНИ висоти під footprint — будинок сідає на свій
                # ПРИРОДНИЙ рівень землі. Раніше брали np.min → на схилі будинок «тонув»
                # у найнижчу точку (верхній бік схилу височів над дахом, видно лише
                # частину висоти; скарга власника). Медіана: пад на серединному рівні,
                # будинок видно на повну висоту; стабільна до викидів (на відміну від max,
                # що давав зависання). База будинку семплить цей же пад → без щілин.
