    ...
    try:
        ...
        for g in building_geometries:
            for poly in _iter_polygons(g):
                ...
                # Вирівнюємо до мінімальної висоти — будинок ставиться
                # у найнижчу точку, не плаває на низькому боці схилу.
                # На верхньому боці terrain вище за будинок (природна виїмка в схилі).
                ref = float(np.min(h))
                Z_out[mask] = ref
        return Z_out
