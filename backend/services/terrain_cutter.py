    ...
    for mesh in building_meshes:
        ...
        try:
            ...
            if not np.any(bottom_face_mask):
                continue
            bottom_vertex_indices = np.unique(faces[bottom_face_mask].ravel())
            # Only LOWER bottoms toward target_z, never RAISE (connector notch_clearance
            # on a high-floor/relief DEM can put target_z ABOVE a building bottom → the
            # old assignment lifted it off the terrain = floating buildings). min() keeps
            # seated buildings put; taller ones still extend down to the base.
