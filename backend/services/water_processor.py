...
def process_water_surface(
    ...
    # Seam-stable texture parameters. Noise is evaluated in world coordinates so
    # adjacent tiles share the same phase at borders.
    ADD_TEXTURE = True
    NOISE_SCALE = max(model_mm_to_world_m(3.2, scale_factor), 4.0) if scale_factor and scale_factor > 0 else 12.0
    NOISE_AMPLITUDE = (
        min(max(model_mm_to_world_m(0.30, scale_factor), float(thickness_m) * 0.30), model_mm_to_world_m(0.40, scale_factor))
        if scale_factor and scale_factor > 0
        else min(max(float(thickness_m) * 0.30, 0.10), 2.0)
    )

    def get_noise(x, y):
        v1 = np.sin(x / NOISE_SCALE * 1.9 + 0.23)
        v2 = np.cos(y / NOISE_SCALE * 2.7 - 0.61)
        v3 = np.sin((x + y) / NOISE_SCALE * 1.3 + 1.07)
        return (v1 + v2 + v3) * (NOISE_AMPLITUDE / 3.0)


    if global_center is not None and not coordinates_already_local:
        try:
        ...
        for poly in polys:
            ...
            try:
                ...
                if terrain_provider is not None and len(v) > 0:
                    ...
                    # Constraint: Water at depressed_ground or slightly below. If bridge/building on water - don't clamp.
                    clamp_limit = depressed_ground - 0.001  # дозволяємо воду до 1мм нижче дна
